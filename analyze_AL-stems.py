...
Description:
------------
This script analyzes RNA apical loop vs fully-BP stem length data stored in files with three columns:
1. length of th AL
2. 3'-end sequence of the stem (string)
3. 5'-end sequence of the stem (string)

It performs the following:
1. Reads all files from the specified directory (`results/stemloops/fully_BP_stemLoops/ALvsSTEMlengh`).
...
Input:
------
Text files named in the format: `AL_XXXnt_YYYxZZZstem.txt`
  - XXX: Length of AL
...
def parse_filename(filename):
    ...
    """
    filename = filename.replace("AL_", "").replace("stem.txt", "")
...
def sort_data_matrix(data):
    ...
    occurrences = occurrences.reindex(sorted(occurrences.columns, key=int), axis=1)
    mean_distances = mean_distances.reindex(sorted(mean_distances.columns, key=int), axis=1)
    std_distances = std_distances.reindex(sorted(std_distances.columns, key=int), axis=1)
#    occurrences = occurrences.sort_index(axis=1, ascending=False)
#    mean_distances = mean_distances.sort_index(axis=1, ascending=False)
#    std_distances = std_distances.sort_index(axis=1, ascending=False)
...
def create_heatmaps(data, output_folder):
    ...
    relative_occurrences = occurrences / tot_seq *100
    ...
    sns.heatmap(occurrences, annot=True, fmt=".1f", cmap="Blues", cbar_kws={'label': 'Occurrences'})
    plt.title("Absolute Occurrences Heatmap")
    plt.savefig(os.path.join(output_folder, "absolute_occurrences.png"))
    plt.close()

    plt.figure(figsize=(12, 6))
    sns.heatmap(relative_occurrences, annot=True, fmt=".2f", cmap="Purples", cbar_kws={'label': 'Relative Occurrences'})
    plt.title("Relative Occurrences Heatmap")
    plt.savefig(os.path.join(output_folder, "relative_occurrences.png"))
    plt.close()
#
#   plt.figure(figsize=(12, 6))
#   sns.heatmap(mean_distances, annot=True, fmt=".2f", cmap="Greens", cbar_kws={'label': 'Mean Distance'})
#   plt.title("Mean Distances Heatmap")
#   plt.savefig(os.path.join(output_folder, "mean_distances.png"))
#   plt.close()
#
#   plt.figure(figsize=(12, 6))
#   sns.heatmap(std_distances, annot=True, fmt=".2f", cmap="Oranges", cbar_kws={'label': 'Standard Deviation'})
...
if __name__ == "__main__":
    # Count the number of lines in the file without loading it into memory
    with open("Zavg_-2_top2000.txt", "r") as file:
        tot_seq = sum(1 for line in file)

    input_folder = "results/stemloops/fully_BP_stemLoops/ALvsSTEMlength"
